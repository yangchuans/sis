<ion-view view-title="设备启停统计">
	<ion-nav-buttons side="left">
        <button menu-toggle="left" class="button button-icon icon ion-navicon"></button>
    </ion-nav-buttons>
    <ion-content>
   		 <div id="stastoPie" style="width:100%;height:370px;"></div>
   		 <div class="row row-head responsive-break" style="background:#5793f3;margin-top: -33px;">
			      <div class="col col-33" >
			        <div class="col-item row-head-item" >持续时间</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item row-head-item">停止</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item row-head-item">启动</div>
			      </div>
		   </div>
		   <div class="row row-body responsive-break">
			      <div class="col col-33" >
			        <div class="col-item" >3小时</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_red">05-11 09:08</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_blue">05-11 12:08</div>
			      </div>
		   </div>
		    <div class="row row-body responsive-break">
			      <div class="col col-33" >
			        <div class="col-item" >3小时</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_red">05-11 14:18</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_blue">05-11 19:18</div>
			      </div>
		   </div>
		    <div class="row row-body responsive-break">
			      <div class="col col-33" >
			        <div class="col-item" >3小时</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_red">05-11 21:23</div>
			      </div>
			      <div class="col col-33" >
			        <div class="col-item status_blue">05-11 23:23</div>
			      </div>
		   </div>
     </ion-content>
</ion-view>